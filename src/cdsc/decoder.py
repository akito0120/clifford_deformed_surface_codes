...
def resolve_decoder(name: str):
    if name == "mwpm":
        return "pymatching", None
    elif name == "bp":
        return "bp", {"bp": BeliefMatchingSinterDecoder()}
    elif name == "uf":
        return "uf", {"uf": UnionFindSinterDecoder()}
    raise ValueError(f"invalid decoder specified: {name!r}")
